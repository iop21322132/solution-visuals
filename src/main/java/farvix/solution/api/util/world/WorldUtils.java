package farvix.solution.api.util.world;

import farvix.solution.mixins.accessors.IGameRenderer;
...
public class WorldUtils {
    ...
    public static final Matrix4f lastWorld = new Matrix4f(), lastProj = new Matrix4f(), lastModelView = new Matrix4f();

    public static Vec3d getPosition(Vec3d pos) {
    ...
    public static double getScale(Vec3d position, double size) {
        Vec3d cam = mc.gameRenderer.getCamera().getPos();
        double distance = cam.distanceTo(position);
        double fov = ((IGameRenderer) mc.gameRenderer).getFov$solution(mc.gameRenderer.getCamera(), mc.getRenderTickCounter().getTickDelta(true), true);
