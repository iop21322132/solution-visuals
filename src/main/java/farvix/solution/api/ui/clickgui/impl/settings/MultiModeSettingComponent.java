...
@Getter
public class MultiModeSettingComponent extends SettingComponent {
    
    private MultiModeSetting multiModeSetting;
    private final List<SubComponent> subComponents = new ArrayList<>();
    private float rightMargin = 5;
    private float notExpandedHeight = 18;
    
    public MultiModeSettingComponent(Setting setting, ModuleComponent moduleComponent) {
    ...
    public void init() {
        ...
        if (!(getSetting() instanceof MultiModeSetting)) {
            throw new IllegalArgumentException("MultiModeSettingComponent only supports MultiModeSetting, got: " + getSetting().getClass().getSimpleName());
        }
        
        this.multiModeSetting = (MultiModeSetting) getSetting();
        List<BooleanSetting> boolSettings = multiModeSetting.getBoolSettings();
    ...
    public float getHeight() {
        ...
        for (SubComponent component : subComponents) {
            String name = farvix.solution.api.translation.Translations.tr(component.getSetting().getName());
            float btnW = Fonts.DEFAULT.get(13).getStringWidth(name) + 14f;
