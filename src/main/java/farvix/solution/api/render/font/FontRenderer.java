...
import farvix.solution.api.render.font.entry.DrawEntry;
import farvix.solution.api.render.font.glyph.Glyph;
import farvix.solution.api.render.font.glyph.GlyphMap;
import farvix.solution.api.util.math.MathUtility;
import farvix.solution.api.util.other.StringUtility;
...
public class FontRenderer implements QuickImports {
    ...
    private Glyph locateGlyph(char glyph) {
        ...
        char base = (char) MathUtility.floorNearestMulN(glyph, 128);
    ...
    public void drawText(MatrixStack matrix, Text text, double x, double y) {
        ...
        findStyle(sb,text);
        drawString(matrix, sb.toString(), x, y, Color.WHITE.getRGB());
    }

    public void findStyle(StringBuilder sb, Text component) {
        Style style = component.getStyle();
        if (component.getSiblings().isEmpty()) {
            if (style.getColor() != null) sb.append(formatting(style.getColor().getRgb()));
    ...
    public void drawStringWithScroll(MatrixStack matrix, String text, double x, double y, float width, int color) {
        ...
        drawString(matrix,text + separation + text, x - MathUtility.textScrolling(textWidth), y, color);
    }
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (c == '§') {
                ...
                if (colorCodes.containsKey(c1)) {
                    clr = new Color(colorCodes.get(c1)).getRGB();
                } else if (c1 == 'R') {
                    clr = color;
                }
                continue;
    ...
    public void drawBoldString(MatrixStack matrix, String text, double x, double y, int color) {
        drawString(matrix, text, x, y, color);
    }

    public void drawCenteredBoldString(MatrixStack matrix, String text, double x, double y, int color) {
        drawCenteredString(matrix, text, x, y, color);
    }
    ...
    public static @NotNull Identifier randomIdentifier() {
        return Identifier.of("zenith", "temp/" + StringUtility.randomString(32));
    }
