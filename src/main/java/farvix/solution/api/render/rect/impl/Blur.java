...
public class Blur implements Shape, QuickImports {
    private final ShaderProgramKey SHADER_KEY = new ShaderProgramKey(Identifier.of("solution", "core/blur"), VertexFormats.POSITION, Defines.EMPTY);
    public Framebuffer input;
    public Vector2f resolution = new Vector2f();
    
    // Кэшируем Vector3f для переиспользования (избегаем аллокаций каждый кадр)
    ...
    // Флаг — шейдер не поддерживается (VulkanMod и т.д.)
    private static boolean shaderUnsupported = false;

    @Override
    public void render(ShapeProperties shape) {
        if (shaderUnsupported) return; // пропускаем если шейдер не работает
        try {
            renderInternal(shape);
        } catch (Exception e) {
            shaderUnsupported = true; // помечаем как неподдерживаемый
        }
    }
    ...
    private void renderInternal(ShapeProperties shape) {
        ...
        // Переиспользуем кэшированные векторы вместо создания новых
