...
public abstract class HudElement extends Module {
    ...
    private float baseScreenWidth = -1f, baseScreenHeight = -1f;
...
//            e.getContext().drawTextWithShadow(mc.textRenderer, text, x, y, 0xFFFFFF);
    ...
    public void setBounds(float x, float y, float width, float height) {
        ...
        float normalizedX = position.getValue().getX();
        float normalizedY = position.getValue().getY();
        ...
        if (sw > 0f && sh > 0f) {
            if (baseScreenWidth < 0f || baseScreenHeight < 0f) {
                ...
                if (sw > baseScreenWidth) baseScreenWidth = sw;
                if (sh > baseScreenHeight) baseScreenHeight = sh;
            }

            designWidth = width;
            designHeight = height;

            float scaleX = baseScreenWidth > 0f ? sw / baseScreenWidth : 1f;
            float scaleY = baseScreenHeight > 0f ? sh / baseScreenHeight : 1f;
            scaleFactor = Math.min(1f, Math.min(scaleX, scaleY));

            this.width = width * scaleFactor;
            this.height = height * scaleFactor;

            float maxX = Math.max(0f, sw - this.width);
            float maxY = Math.max(0f, sh - this.height);
            float clampedX = Math.min(Math.max(x, 0f), maxX);
            float clampedY = Math.min(Math.max(y, 0f), maxY);
            normalizedX = clampedX / sw;
            normalizedY = clampedY / sh;
        } else {
            scaleFactor = 1f;
            designWidth = width;
            designHeight = height;
            ...
            if (sw > 0f) {
                float maxRatioX = Math.max(0f, (sw - width) / sw);
                normalizedX = Math.min(Math.max(x / sw, 0f), maxRatioX);
            }

            if (sh > 0f) {
                float maxRatioY = Math.max(0f, (sh - height) / sh);
                normalizedY = Math.min(Math.max(y / sh, 0f), maxRatioY);
            }
        }

        position.getValue().setX(normalizedX);
        position.getValue().setY(normalizedY);
