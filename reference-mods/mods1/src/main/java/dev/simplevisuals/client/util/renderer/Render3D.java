...
public class Render3D implements Wrapper {
    ...
    /**
     * Установить текущие матрицы преобразования (должен вызываться в начале рендеринга мира)
     */
    public static void setMatrices(MatrixStack matrixStack, Matrix4f projectionMatrix) {
        if (matrixStack != null) {
            lastWorldSpaceMatrix = matrixStack.peek();
        }
        if (projectionMatrix != null) {
            lastProjMat = projectionMatrix;
        }
    }

    /**
     * Преобразовать мировые координаты в координаты камеры
     */
    private static Vec3d toCameraSpace(Vec3d worldPos) {
    ...
    public static void drawBlockOverlayLine(Vec3d start, Vec3d end, int color) {
        ...
        Vec3d s = toCameraSpace(start);
        Vec3d e = toCameraSpace(end);

        Matrix4f matrix = lastWorldSpaceMatrix.getPositionMatrix();
    ...
    public static void drawQuadsWithDepth(List<VertexCollection> quads, boolean shine) {
        ...
        // Аддитивный блендинг для всех квадов
        RenderSystem.enableBlend();
        RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE);
    ...
    public static void drawTexture(MatrixStack matrices, float x0, float y0, float z0,
        ...
        // Аддитивный блендинг для текстур
        RenderSystem.enableBlend();
        RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE);
    ...
    public static void beginBillboardBatch(Identifier texture) {
        ...
        // Аддитивный блендинг для батчинга
        RenderSystem.enableBlend();
        RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE);
    ...
    public static void drawBillboardTexture(MatrixStack matrices, Vec3d worldPos, float size, Identifier texture, Color color) {
        drawBillboardTexture(matrices, worldPos, size, texture, color, true, 0f);
    }

    public static void drawBillboardTexture(MatrixStack matrices, Vec3d worldPos, float size, Identifier texture, Color color, boolean additiveBlend) {
        drawBillboardTexture(matrices, worldPos, size, texture, color, additiveBlend, 0f);
    }

    public static void drawBillboardTexture(MatrixStack matrices, Vec3d worldPos, float size, Identifier texture, Color color, boolean additiveBlend, float rotationDegrees) {
        RenderSystem.setShader(ShaderProgramKeys.POSITION_TEX_COLOR);
        RenderSystem.setShaderTexture(0, texture);

        // Блендинг для billboard
        RenderSystem.enableBlend();
        if (additiveBlend) {
            RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE);
        } else {
    ...
    public static void drawFixedBillboardTexture(MatrixStack matrices, Vec3d worldPos, float size, Identifier texture, Color color, float fixedRotation) {
        ...
        // Аддитивный блендинг для фиксированного billboard
        RenderSystem.enableBlend();
        RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE);
    ...
    public static void drawSimpleBillboardTexture(MatrixStack matrices, float size, Identifier texture, Color color) {
        ...
        // Аддитивный блендинг для упрощенного billboard
        RenderSystem.enableBlend();
        RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE);
    ...
    public static void draw(List<VertexCollection> quads, List<VertexCollection> debugLines, boolean shine) {
        ...
        // Аддитивный блендинг для всего рендера
        RenderSystem.enableBlend();
        RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE);
        ...
        if (!debugLines.isEmpty()) {
            ...
            GL11.glHint(GL11.GL_LINE_SMOOTH_HINT, GL11.GL_NICEST);
            GL11.glEnable(GL11.GL_LINE_SMOOTH);
            // Убрано GL11.glLineWidth() - вызывает ошибку OpenGL 1281 в preview context
            BufferRenderer.drawWithGlobalProgram(buffer.end());
            GL11.glDisable(GL11.GL_LINE_SMOOTH);
        }
    ...
    public static void drawIgnoringDepth(List<VertexCollection> quads, List<VertexCollection> debugLines, boolean shine) {
        ...
        // Аддитивный блендинг для рендера без глубины
        RenderSystem.enableBlend();
        RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE);
    ...
    private static Vec3d cameraTransform(Vec3d vec3d) {
        ...
        vec.mul(lastWorldSpaceMatrix.getPositionMatrix());
    ...
    public static void drawTextureVivid(MatrixStack matrices, float x0, int y0, float z0,
                                        float x1, int y1, float z1, Identifier texture, Color color, float strength) {
        drawTextureVivid(matrices, x0, (float) y0, z0, x1, (float) y1, z1, texture, color, strength, VividMode.SOFT);
    }
