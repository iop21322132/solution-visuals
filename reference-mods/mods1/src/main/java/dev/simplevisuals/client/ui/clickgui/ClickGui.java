...
public class ClickGui extends Screen implements Wrapper {
    ...
    private final Animation transitionAnimation = new Animation(260, 1f, true, Easing.OUT_QUART);
    ...
    private float mainScrollbarDragOffsetY = 0f;

    private boolean draggingSettingsScrollbar = false;
    private float settingsScrollbarDragOffsetY = 0f;
    ...
    public void init() {
        super.init();
        this.width = 320f;
        this.height = calculateDynamicHeight();
        ...
        transitionAnimation.update(true);

        // Воспроизвести звук при открытии
        playSoundClickGui(false);

        UI ui = simplevisuals.getInstance() != null && simplevisuals.getInstance().getModuleManager() != null
                ? simplevisuals.getInstance().getModuleManager().getModule(UI.class)
                : null;
        boolean snowEnabled = ui != null && ui.isClickGuiSnowEnabled();
        if (snowEnabled) ensureSnowflakes();
        else snowflakes.clear();
    }


    private float calculateDynamicHeight() {
        float baseHeight = 300f;
        float maxHeight = mc.getWindow().getScaledHeight() - 100f; // Leave some margin
        float contentHeight = estimateContentHeight();
        
        // If content needs more space, expand up to max height
        if (contentHeight > baseHeight - 100f) {
            return Math.min(baseHeight + (contentHeight - (baseHeight - 100f)), maxHeight);
        }
        return baseHeight;
    }
    ...
    private float estimateContentHeight() {
        ...
        for (Category cat : TABS) {
            if (cat == Category.Theme) {
                // Estimate themes height
                estimatedHeight += themeManager.getAvailableThemes().length * 20f;
            } else {
                // Estimate modules height for this category
                List<Module> mods = simplevisuals.getInstance().getModuleManager().getModules(cat);
                estimatedHeight += mods.size() * 30f; // Rough estimate per module
            }
        }
    ...
    private float calculateSettingsPanelHeight(ModuleComponent target) {
        if (target == null || target.getComponents().isEmpty()) {
            return 200f; // Default minimum height
        }
        
        // Estimate height based on number of settings
        // Each setting typically needs about 25-30 pixels of space
        float estimatedHeight = target.getComponents().size() * 28f;
        return Math.max(200f, Math.min(estimatedHeight, 600f)); // Min 200, max 600
    }
    ...
    private void playSoundClickGui(boolean wasToggled) {
        ClientSound clientSound = simplevisuals.getInstance().getModuleManager().getModule(ClientSound.class);
        if (clientSound != null && clientSound.clickguisound.getValue() && clientSound.isToggled()) {
            String soundId = wasToggled ? clientSound.getOffClickguiSoundId() : clientSound.getOnClickguiSoundId();
            float volume = clientSound.getVolumeClickgui().getValue();
            MinecraftClient.getInstance().getSoundManager().play(
                    PositionedSoundInstance.master(
                            SoundEvent.of(Identifier.of(soundId)),
                            1.0f,
    ...
    public void close() {
        if (closing) return;
        closing = true;
        transitionAnimation.update(false);
        playSoundClickGui(true);
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        UI ui = simplevisuals.getInstance() != null && simplevisuals.getInstance().getModuleManager() != null
                ? simplevisuals.getInstance().getModuleManager().getModule(UI.class)
                : null;
        boolean bgBlurEnabled = ui == null || ui.isClickGuiBlurEnabled();
        boolean snowEnabled = ui != null && ui.isClickGuiSnowEnabled();

        transitionAnimation.update(!closing);
        float t = Math.max(0f, Math.min(1f, transitionAnimation.getValue()));
        uiAlpha = t;

        // Finish closing: close screen first, then toggle module off (avoids onDisable recursion)
        if (closing && t <= 0.01f) {
            if (mc.currentScreen == this) {
                mc.setScreen(null);
            }
            if (ui != null && ui.isToggled()) {
                ui.setToggled(false);
            }
            return;
        }

        // New animation: fade + slight scale + slide
        float scale = 0.88f + 0.12f * t;
        float slideY = (1f - t) * 18f;

        float baseW = 320f;
        float baseH = calculateDynamicHeight();
        float drawW = baseW * scale;
        float drawH = baseH * scale;
        this.width = drawW;
        this.height = drawH;
        this.x = (mc.getWindow().getScaledWidth() - this.width) / 2f;
        this.y = (mc.getWindow().getScaledHeight() - this.height) / 2f + slideY;

        contentOffsetY = (1f - t) * 10f;

        if (bgBlurEnabled) {
            float blurAlpha = 160f * uiAlpha;
            if (blurAlpha > 1f) {
                Render2D.drawShaderBlurRect(
                        ...
                        0f,
                        8f,
                        new Color(255, 255, 255, (int) blurAlpha)
                );
            }
        }
        ...
        if (snowEnabled) {
            ...
            ensureSnowflakes();
            updateSnowflakes(dt);
            snowTimeSec = nowNanos / 1_000_000_000f;
        } else {
            lastSnowUpdateNanos = 0L;
        }

        // Snow behind ClickGUI (background layer, not on the panel)
        if (snowEnabled) {
            renderSnowflakes(context, snowTimeSec);
        }
    ...
    private void ensureSnowflakes() {
        ...
        if (w <= 0 || h <= 0) return;
        int targetCount = Math.min(320, Math.max(110, (w * h) / 11000));
        if (snowflakes.size() == targetCount) return;

        snowflakes.clear();
        for (int i = 0; i < targetCount; i++) {
            Snowflake flake = new Snowflake();
            flake.respawn(snowRandom, w, h, true);
    ...
    private void updateSnowflakes(float dt) {
        int w = mc.getWindow().getScaledWidth();
        int h = mc.getWindow().getScaledHeight();
        if (snowflakes.isEmpty() || w <= 0 || h <= 0) return;
        for (Snowflake flake : snowflakes) {
            flake.y += flake.speed * dt;
            flake.x += flake.wind * dt;

            float padding = 24f;
            if (flake.x < -padding) flake.x = w + padding;
            if (flake.x > w + padding) flake.x = -padding;
            if (flake.y - flake.size > h + padding) {
                flake.respawn(snowRandom, w, h, false);
            }
        }
    }

    private void renderSnowflakes(DrawContext ctx, float timeSec) {
        if (snowflakes.isEmpty() || uiAlpha <= 0f) return;
        for (Snowflake flake : snowflakes) {
            int a = (int) (255f * flake.opacity * uiAlpha * 1.6f);
            if (a <= 0) continue;
            float sx = flake.x + (float) Math.sin(timeSec * flake.swaySpeed + flake.phase) * flake.swayAmp;
            float sy = flake.y;
            Render2D.drawRoundedRect(ctx.getMatrices(), sx, sy, flake.size, flake.size, flake.size,
                    new Color(235, 235, 255, Math.min(255, a)));
        }
    }
    ...
    private static final class Snowflake {
        ...
        void respawn(Random random, int width, int height, boolean randomY) {
            this.size = 2.2f + random.nextFloat() * 3.6f;
            this.speed = 22f + random.nextFloat() * 72f;
            ...
            this.opacity = 0.45f + random.nextFloat() * 0.45f;
    ...
    private void renderModulesArea(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        if (anim > 0.01f) {
            ...
            // Calculate dynamic panel height based on settings content
            float dynamicPanelH = calculateSettingsPanelHeight(target);
            float panelH = Math.min(dynamicPanelH, height - 200f); // Cap at available space
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        // Drag scrollbar (modules/themes list)
        if (button == 0 && maxScroll > 0.5f) {
            float trackX = x + width - 4f;
            float trackY = leftY;
            float trackW = 2f;
            float trackH = leftH;
            ...
            float thumbX = trackX - 0.5f;
            float thumbW = trackW + 1f;
            boolean overThumb = mouseX >= thumbX && mouseX <= thumbX + thumbW && mouseY >= thumbY && mouseY <= thumbY + thumbH;
            boolean overTrack = mouseX >= thumbX && mouseX <= thumbX + thumbW && mouseY >= trackY && mouseY <= trackY + trackH;
            if (overThumb || overTrack) {
                if (!overThumb) {
                    float pos = (float) mouseY - (thumbH * 0.5f);
                    float r = (pos - trackY) / Math.max(1f, maxThumbTravel);
                    r = clamp(r, 0f, 1f);
                    float newScroll = r * maxScroll;
                    scrollY = newScroll;
                    scrollYTarget = newScroll;
                    thumbY = trackY + maxThumbTravel * r;
                }
                draggingMainScrollbar = true;
                mainScrollbarDragOffsetY = (float) mouseY - thumbY;
                return true;
            }
        }
        ...
        if (activeSettings != null && !activeSettings.getComponents().isEmpty()) {
            ...
            if (anim > 0.2f) {
                ...
                float panelW = 120f;
                float dynamicPanelH = calculateSettingsPanelHeight(activeSettings);
                float panelH = Math.min(dynamicPanelH, height - 200f);
                float slideOffset = (1f - anim) * 40f;
                float drawPanelX = basePanelX + slideOffset;
                float rContentX = drawPanelX + 8f;
                float rContentY = panelY + 8f;
                ...
                // Drag scrollbar (settings panel)
                if (button == 0 && settingsMaxScroll > 0.5f) {
                    float trackX = drawPanelX + panelW - 3f;
                    float trackY = rContentY;
                    float trackW = 2f;
                    float trackH = rContentH;
                    ...
                    if (overThumb || overTrack) {
                        if (!overThumb) {
                            ...
                            float r = (pos - trackY) / Math.max(1f, maxThumbTravel);
                            r = clamp(r, 0f, 1f);
                            float newScroll = r * settingsMaxScroll;
                            settingsScrollY = newScroll;
                            settingsScrollYTarget = newScroll;
                            thumbY = trackY + maxThumbTravel * r;
                        }
                        draggingSettingsScrollbar = true;
                        settingsScrollbarDragOffsetY = (float) mouseY - thumbY;
                        return true;
                    }
                }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        if (button == 0 && draggingMainScrollbar && maxScroll > 0.5f) {
            float tabH = 18f;
            float leftY = y + 10f + tabH + 9f + contentOffsetY;
            float leftH = height - ((leftY - y) + 8f);
            float trackY = leftY;
            ...
            float pos = (float) mouseY - mainScrollbarDragOffsetY;
            float r = (pos - trackY) / Math.max(1f, maxThumbTravel);
            r = clamp(r, 0f, 1f);
            float newScroll = r * maxScroll;
            scrollY = newScroll;
        ...
        if (button == 0 && draggingSettingsScrollbar && settingsMaxScroll > 0.5f && activeSettings != null && !activeSettings.getComponents().isEmpty()) {
            float anim = settingsAnimation.getValue();
            if (anim > 0.2f) {
                float panelY = y + 90f + contentOffsetY;
                float dynamicPanelH = calculateSettingsPanelHeight(activeSettings);
                ...
                float pos = (float) mouseY - settingsScrollbarDragOffsetY;
                float r = (pos - trackY) / Math.max(1f, maxThumbTravel);
                r = clamp(r, 0f, 1f);
                float newScroll = r * settingsMaxScroll;
                settingsScrollY = newScroll;
