...
public class TargetEsp extends Module implements ThemeManager.ThemeChangeListener {
    ...
    private final BooleanSetting markerAdditiveBlending = new BooleanSetting("setting.additiveBlending", false, () -> modeMarker.getValue());
    private final BooleanSetting markerHitFlash = new BooleanSetting("setting.hitFlash", true);
    ...
    public TargetEsp() {
        ...
        markerAdditiveBlending.setVisible(() -> modeMarker.getValue());
    ...
    public void onThemeChanged(ThemeManager.Theme theme) {
        // Color is now fetched dynamically for gradient support
    }
    ...
    private Color getMarkerColor() {
        ...
        // Use current theme color for gradient support
        Color themeColor = themeManager.getCurrentTheme().getBackgroundColor();
        return new Color(themeColor.getRed(), themeColor.getGreen(), themeColor.getBlue(), 255);
    }
    ...
    private void renderMarkerMain(EventRender2D e, Vec3d centerWorld, float animVal, int colorRGBA) {
        ...
        e.getContext().getMatrices().push();
        e.getContext().getMatrices().translate(0, 0, -200);
        ...
        Render2D.drawTexture(
                ...
                0f,
                simplevisuals.id(modeMarkerNew.getValue() ? "textures/target.png" : "hud/marker.png"),
                finalColor,
                markerAdditiveBlending.getValue()
        );
