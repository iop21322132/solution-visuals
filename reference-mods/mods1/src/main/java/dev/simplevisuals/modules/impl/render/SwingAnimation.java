...
public class SwingAnimation extends Module {
    ...
    // Settings
    private final BooleanSetting modeFirst = new BooleanSetting("mode.first", true, () -> false);
    ...
    private final ListSetting animationMode = new ListSetting(
            ...
            modeFirst, modeSecond, modeThird, modeFourth,
    ...
    public void renderSwordAnimation(MatrixStack matrices, float swingProgress, float equipProgress, Arm arm) {
        ...
        BooleanSetting activeMode = animationMode.getToggled().stream().findFirst().orElse(modeFirst);
        if (animationMode.getToggled().size() > 1) {
            ...
            // If no modes are toggled, enable First
            modeFirst.setValue(true);
            activeMode = modeFirst;
        }
