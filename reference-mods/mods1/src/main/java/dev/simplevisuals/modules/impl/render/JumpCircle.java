    ...
    public void onRender3D(EventRender3D.Game e) {
        ...
        try (var __ = Perf.scopeCpu("JumpCircle.onRender3D")) {
            ...
            for (Circle c : circles) {
                ...
                // Нормированная величина времени жизни 0..1
                float lifeProgress = Math.max(0f, Math.min(1f, age / (float) c.ttl));
                // Скорость влияет на анимацию, но не «обрезает» время исчезновения
                float animProgress = Math.max(0f, Math.min(1f, lifeProgress * speed.getValue()));
                ...
                if (animPulse.getValue()) {
                    ...
                    float s = (float) Math.sin(Math.PI * pulses * animProgress);
                    radiusMul = 0.7f + 0.5f * Math.max(0f, s);
                    alphaK = Math.max(0f, s);
                } else if (animRipple.getValue()) {
                    // Ripple: мягкое появление/исчезновение и ease-out по радиусу
                    float te = Easing.EASE_OUT_CIRC.apply(animProgress);
                    radiusMul = 0.6f + 0.8f * te;
                    alphaK = (float) Math.pow(Math.sin(animProgress * (float) Math.PI), 1.2f);
                } else {
                    // Grow: используем плавный S-curve по времени + эксп. затухание
                    float te = Easing.BOTH_SINE.apply(animProgress);
                    radiusMul = 0.6f + 0.4f * te;
                    alphaK = (float) Math.pow(1.0f - animProgress, 0.8f) * te;
                }

                float lifetimeFade = (float) Math.pow(1.0f - lifeProgress, 0.9f);
                alphaK = Math.max(0f, Math.min(1f, alphaK * lifetimeFade));
