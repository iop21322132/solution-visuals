...
public class DamageParticles extends Module implements ThemeManager.ThemeChangeListener {
    ...
    private final NumberSetting dmgSize       = new NumberSetting("setting.size", 3f, 1f, 5f, 1f);
    ...
    private final NumberSetting rotationSpeedMultiplier = new NumberSetting("setting.rotationSpeed", 1.0f, 0f, 3.0f, 0.05f);
    ...
    public void onRender3D(EventRender3D.Game e) {
        ...
        for (int i = 0, size = particles.size(); i < size; i++) {
            ...
            float worldSize = dmgSize.getValue() * 0.04f * 0.3f * f * 8f;
            ...
            Render3D.drawBillboardTexture(e.getMatrices(), renderPos, worldSize, p.tex, color, true, p.rotation);
        }
    }
    ...
    private static class Particle {
        ...
        Particle(DamageParticles owner, boolean legacy, Vec3d pos, Vec3d vel, Identifier tex, long lifeTime){
            ...
            float speed = (ThreadLocalRandom.current().nextFloat() - 0.5f) * 18f;
            this.rotationSpeed = Math.abs(speed) < 1.5f ? Math.copySign(1.5f, speed == 0 ? 1 : speed) : speed;
        }
        ...
        static Particle createLegacy(DamageParticles owner, Vec3d pos, Identifier tex, long lifeTime, float scatter) {
            ...
            double sy = u;
            double sz = r * Math.sin(theta);
            // Изменено: ВЗЛЕТАЮТ - положительная вертикальная составляющая
            Vec3d dir = new Vec3d(sx, Math.abs(sy), sz).normalize(); // Убрано отрицание
        ...
        void updatePhysics() {
            ...
            float rotationScale = owner.rotationSpeedMultiplier.getValue();
            rotation += rotationSpeed * s * rotationScale;
